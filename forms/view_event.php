...
<BODY>
<?php
	$query2 = "SELECT * FROM `$GLOBALS[mysql_prefix]events` WHERE `id` = '" . $skill_id . "' LIMIT 1";
	...
	<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
		<DIV id = "leftcol" style='position: relative; left: 30px; top: 70px; float: left;'>
			<DIV id='button_bar' class='but_container'>
				<SPAN CLASS='heading' STYLE='text-align: center; display: inline; font-size: 1.5em;'>View <?php print get_text('Event');?> Attended by "<?php print $row['field2'];?> <?php print $row['field1'];?>"</SPAN>
			...
			<FORM METHOD="POST" NAME= "tpack_view_Form" ACTION="#">
				<FIELDSET>
				<LEGEND><?php print get_text('Event');?> Attendance</LEGEND>
					<BR />
					<LABEL><?php print get_text('Event');?> Attended:</LABEL>
					...
					<BR />
					<LABEL><?php print get_text('Event Type');?>:</LABEL>
					<DIV class='td_data_wrap'><?php print $type;?></DIV>
					<BR />
				</FIELDSET>
			</FORM>	
		</DIV>
