...
function rem_record() {
	if (confirm("Are you sure you want to delete the Record")) { 
		document.clothing_edit_Form.frm_all_remove.value="yes";
		document.forms['clothing_edit_Form'].submit();
		}
	}
</SCRIPT>
</HEAD>
<BODY>	
	<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
		<DIV CLASS='header text_large' style = "height:32px; width: 100%; float: none; text-align: center;">
			<SPAN ID='theHeading' CLASS='header text_bold text_big' STYLE='background-color: inherit;'><b>Edit <?php print get_text('Clothing');?> for "<?php print $row['field2'];?> <?php print $row['field1'];?>"</b></SPAN>
		</DIV>
		<DIV id = "leftcol" style='position: relative; left: 30px; float: left;'>
			<FORM METHOD="POST" NAME= "clothing_edit_Form" ACTION="member.php?func=member&goeditcloth=true&extra=edit">
				<FIELDSET>
				<LEGEND><?php print get_text('Clothing');?> Issued</LEGEND>
					<BR />
...
<?php
							$query_clo = "SELECT * FROM `$GLOBALS[mysql_prefix]clothing_types` ORDER BY `clothing_item` ASC";
							$result_clo = mysql_query($query_clo) or do_error($query_clo, 'mysql query failed', mysql_error(),basename( __FILE__), __LINE__);
							while ($row_clo = stripslashes_deep(mysql_fetch_assoc($result_clo))) {
								$sel = ($row_clo['id'] == $row_all['skill_id']) ? "SELECTED" : "";											
								$text = $row_clo['clothing_item'] . " " . $row_clo['size'];
								print "\t<OPTION class='normalSelect' VALUE='{$row_clo['id']}' {$sel}>{$text}</OPTION>\n";
								}
?>
						</SELECT>
					<BR />
