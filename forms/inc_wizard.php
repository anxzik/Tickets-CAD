...
function generate_date_select($date_suffix, $default_date=0, $theLabel, $disabled=FALSE) {
	...
	$output = "";
	$output .= "<LABEL onmouseout='UnTip();' onmouseover='Tip(\"" . $date_suffix . "\");'>" . $theLabel . ":";
	...
	$output .= "<INPUT TYPE='text' id='wiz_frm_hour_" . $date_suffix . "'' SIZE='2' MAXLENGTH='2' tabindex=" . $tabindex . " NAME='wiz_frm_hour_" . $date_suffix . "' VALUE='" . $hour . "' " . $dis_str . " onChange='set_controlvalue(\"frm_hour_" . $date_suffix . "\", this.selectedIndex);'>:";
	$tabindex++;
	$output .= "<INPUT TYPE='text' id='wiz_frm_minute_" . $date_suffix . "'' SIZE='2' MAXLENGTH='2' tabindex=" . $tabindex . " NAME='wiz_frm_minute_" . $date_suffix . "' VALUE='" . $minute . "' " . $dis_str . " onChange='set_controlvalue(\"frm_minute_" . $date_suffix . "\", this.selectedIndex);'>";
	$tabindex++;
...
function get_intypes_control() {
	...
	$theControl .= "</SELECT>";
	$theControl .= "<BR />";
	$theControl .= "<LABEL for='wiz_proto_cell' style='display: inline;' onmouseout='UnTip();' onmouseover=\"Tip('" . $titles["_proto"] . "');\">" . get_text("Protocol") . " :</LABEL>";
	$theControl .= "<SPAN ID='wiz_proto_cell' style='display: inline-block; color: #000000; padding: 5px; max-height: 50px; overflow-x: auto; width: 55%; float: right;'></SPAN>";
	$theControl .= "<BR /><BR />";
	$tabindex ++;
	return $theControl;
	}
...
function get_address_control() {
	global $titles, $street, $city, $st, $st_size, $addr_sugg_str, $good_internet, $gmaps, $incident, $doloc, $tabindex, $us_states;
	...
	$locale = get_variable('locale');
	if($locale == "0") {
		$theControl .= "<SELECT id='state' NAME='frm_state' tabindex=40 onChange='set_controlvalue(\"wiz_frm_state\",\"frm_state\");'>";
		foreach($us_states as $val) {
			$theControl .= "<OPTION VALUE='" . $val['code'] . "'>" . $val['name'] . " (" . $val['code'] . ")</OPTION>";
			}
		$theControl .= "</SELECT>";
		} else {
