...
<BODY>
<?php
	$query2 = "SELECT * FROM `$GLOBALS[mysql_prefix]clothing_types` WHERE `id` = " . $row_all['skill_id'] . " LIMIT 1";
...
?>
	<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
		<DIV id = "leftcol" style='position: relative; left: 30px; top: 70px; float: left;'>
			...
			<FORM METHOD="POST" NAME= "clothing_edit_Form" ACTION="member.php?func=member&goeditcloth=true">
				<FIELDSET>
				<LEGEND><?php print get_text('Clothing');?></LEGEND>
					<BR />
					<LABEL><?php print get_text('Clothing Issued');?>:</LABEL>
					...
					<BR />
					<LABEL><?php print get_text('Size');?>:</LABEL>
					<DIV class='td_data_wrap'><?php print $size;?></DIV>
					<BR />
				</FIELDSET>
			</FORM>						
		</DIV>
