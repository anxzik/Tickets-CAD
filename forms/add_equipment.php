		...
		<BODY>	
			<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
				<DIV CLASS='header text_large' style = "height:32px; width: 100%; float: none; text-align: center;">
					<SPAN ID='theHeading' CLASS='header text_bold text_big' STYLE='background-color: inherit;'><b>Add <?php print get_text('Equipment');?> for "<?php print $row['field2'];?> <?php print $row['field1'];?>"</b></SPAN>
				</DIV>
				<DIV id = "leftcol" style='position: relative; left: 30px; float: left;'>
					<FORM METHOD="POST" NAME= "equip_add_Form" ACTION="member.php?func=member&goaddequip=true&extra=edit">
						<FIELDSET>
						<LEGEND>Add <?php print get_text('Equipment');?> Issued</LEGEND>
							<BR />
...
<?php
								$query = "SELECT * FROM `$GLOBALS[mysql_prefix]equipment_types` ORDER BY `equipment_name` ASC";
								$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(),basename( __FILE__), __LINE__);
								while ($row = stripslashes_deep(mysql_fetch_assoc($result))) {
									$text = $row['equipment_name'] . " " . $row['spec'];
...
?>
				...
				<DIV ID="middle_col" style='position: relative; left: 40px; width: 110px; float: left;'>&nbsp;
					<DIV style='position: fixed; top: 50px; z-index: 1;'>
						...
						<SPAN ID = 'sub_but' class = 'plain_centerbuttons text' style='width: 80px; display: block; float: none;' onMouseOver="do_hover_centerbuttons(this.id);" onMouseOut="do_plain_centerbuttons(this.id);" onClick="validate_skills(document.equip_add_Form);"><?php print get_text('Save');?> <IMG style='vertical-align: middle;' src="./images/save.png"/></SPAN>			
					</DIV>
				</DIV>
				<DIV id='rightcol' style='position: relative; left: 40px; float: left;'>
					<DIV class='tablehead text_large text_bold text_center'><?php print get_text('Equipment');?></DIV><BR /><BR />				
					<DIV style='padding: 10px; float: left;'><?php print get_text('Equipment');?> Items are those articles of PPE or other classes of <?php print get_text('equipment');?> issued to members.
					<BR />
					...
					<BR />
					<BR />					
					<SPAN style='display: inline-block; float: left;'><?php print get_text('Equipment');?> items need to be added first to the system either from "Config" or by clicking</SPAN>
					<SPAN ID='to_equipment' class = 'plain' style='display: inline-block; float: left;' onMouseOver="do_hover(this.id);" onMouseOut="do_plain(this.id);" onClick = "do_Post('equipment_types');">Here</SPAN><BR />
					</DIV>
				</DIV>
			</DIV>	
