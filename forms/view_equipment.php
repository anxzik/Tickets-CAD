...
<BODY>
<?php
	$query2 = "SELECT * FROM `$GLOBALS[mysql_prefix]equipment_types` WHERE `id` = " . $row_all['skill_id'] . " LIMIT 1";
	...
	<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
		<DIV id = "leftcol" style='position: relative; left: 30px; top: 70px; float: left;'>
			<DIV id='button_bar' class='but_container'>
				<SPAN CLASS='heading' STYLE='text-align: center; display: inline; font-size: 1.5em;'>View <?php print get_text('Equipment');?> Issued to "<?php print $row['field2'];?> <?php print $row['field1'];?>"</SPAN>
