...
//        		$watch_interval = $bits[1];
				...
				$the_ret = "SELECT 'N/A' as `tickstatus`, 
						'Unit Watch' as `tickaddr`,
						'u' as `mode`,
						'' as `type`,
						'' as `tickid`,
						0 as `severity`, 
						`status_updated` as `on_scene`, 
						`status_updated` as `the_asof`,
						'Overdue - Unit' as `unit_status`, 
						unix_timestamp(now()) as `expires`, 
						`handle` as `handle`,
						`contact_via` as `contact_via`,
						`$GLOBALS[mysql_prefix]un_status`.`id` as `unitid`,
						'Overdue Unit' as `scope` 
						...
						WHERE `$GLOBALS[mysql_prefix]un_status`.`watch` = 1 and `status_updated` < now()";
				return $the_ret;
//				} else {
...
//			dump ( $_SESSION['osw_run_at'] );
			$now = ( time() - 30 );								// seconds
			if ( ! (array_key_exists ( "osw_run_at", $_SESSION ) ) ) { $_SESSION['osw_run_at'] = array ( $now-1, $now-1, $now-1, $now-1 ) ; }		// initialize routines, normals, priorities
