...
function set_input_strings () {
	...
	$out_arr['f_1'] = in_text ( 1, 28, " (required)" ) ;		// ( $name, $size, $ph = NULL, $margin=0 )
	...
	for ( $i = 0; $i< ( 6*9 ) ; $i+=6 ) {
			$t = "f_" . strval ( $i+6 ) ;
			$out_arr[$t] = in_area ( strval ( $i+6 ) , 16, 1) ; 		//	( $name, $cols, $rows, $ph = NULL, $margin=0 )

			$t = "f_" . strval ( $i+7 ) ;
			$out_arr[$t] = in_area ( strval ( $i+7 ) , 9, 1) ;

			$t = "f_" . strval ( $i+8 ) ;
			$out_arr[$t] = in_area ( strval ( $i+8 ) , 16, 1) ;

			$t = "f_" . strval ( $i+9 ) ;
			$out_arr[$t] = in_area ( strval ( $i+9 ) , 16, 1) ;

			$t = "f_" . strval ( $i+10 ) ;
			$out_arr[$t] = in_area ( strval ( $i+10 ) , 16, 1 ) ;

			$t = "f_" . strval ( $i+11 ) ;
			$out_arr[$t] = in_area ( strval ( $i+11 ) , 35, 1 ) ;

			}		// end for ()

		$out_arr['f_60'] = in_text ( 60, 88 ) ;		// by
	...
	function merge_template () {		// merge argument array with template -- e.g., <td> $my_inputs_arr['fn] </td>
		...
		$out_str = "\n
			<table id = 'outer' style = 'border-style: none; width: 10.5in;' ><tr><td>		<!-- !!!! -->
			...
			<td style = '{$td_heading}' colspan=1>&nbsp;INCIDENT RADIO<br>&nbsp;&nbsp;&nbsp;COMM'S<br>&nbsp;&nbsp;&nbsp;PLAN&nbsp;&nbsp; &nbsp;&nbsp; ICS 205-CG&nbsp;</td>
			</tr>
			<tr style = '{$tr_thin}'>
			...
			<td style = '{$td_heading} text-align: right;' colspan=1></td>
			</tr>
			</table>
...
<div class="buttons">
 	<SPAN ID='reset_but' 						class='plain text' style='float: none; width: 120px;' onMouseover='do_hover ( this.id ) ;' onMouseout='do_plain ( this.id ) ;' onClick="document.main_form.reset(); document.main_form.f_1.focus();"><SPAN STYLE='float: left;'><?php print get_text ( "Reset" ) ;?></SPAN><IMG STYLE='float: right;' SRC='../images/restore_small.png' BORDER=0></SPAN><BR />
	<SPAN ID='can_but' 							class='plain text' style='float: none; width: 120px;' onMouseover='do_hover ( this.id ) ;' onMouseout='do_plain ( this.id ) ;' onClick="document.can_form.submit () ;"><SPAN STYLE='float: left;'><?php print get_text ( "Cancel" ) ;?></SPAN><IMG STYLE='float: right;' SRC='../images/cancel_small.png' BORDER=0></SPAN><BR />
 	<SPAN ID='save_but' 						class='plain text' style='float: none; width: 120px;' onMouseover='do_hover ( this.id ) ;' onMouseout='do_plain ( this.id ) ;' onClick="save ( document.main_form ) ;">				<SPAN STYLE='float: left;'><?php print get_text ( "Save to DB" ) ;?></SPAN>	<IMG STYLE='float: right;' SRC='../images/restore_small.png' BORDER=0></SPAN><BR />
	<SPAN ID='mail_but' TITLE='OK - Mail this' 	class='plain text' style='float: none; width: 120px;' onMouseover='do_hover ( this.id ) ;' onMouseout='do_plain ( this.id ) ;' onClick="document.main_form.func.value='m';validate ( document.main_form ) ;"><SPAN STYLE='float: left;'><?php print get_text ( "Send" ) ;?></SPAN><IMG STYLE='float: right;' SRC='../images/send_small.png' BORDER=0></SPAN><BR />
<?php
