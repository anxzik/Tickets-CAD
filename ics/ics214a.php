...
<?php echo __LINE__; ?>
*/
define ( "FORM", "ICS 214a" ) ;
define ( "TITLE", "Individual Log");
...
function set_input_strings () {
	$out_arr = array () ;
	$out_arr['f_1'] = in_text ( 1, 22, ' (required)' ) ; 				// $name, $size, placeholder
	$out_arr['f_2'] = in_text ( 2, 12 ) ;
	$out_arr['f_3'] = in_text ( 3, 6 ) ;
	$out_arr['f_4'] = in_text ( 4, 12 ) ;
	$out_arr['f_5'] = in_text ( 5, 6 ) ;

	$out_arr['f_6'] = in_text ( 6, 30 ) ;
	...
	for ( $i=0; $i< ( 2*24 ) ; $i+=2 ) {					// 2 cols, 24 rows ( 0-47 )
		...
		$out_arr[$key] = in_text ( $start + $i+1, 74 ) ; 	// major events
		}

	$out_arr['f_57'] = in_text ( 57, 20 ) ;
	$out_arr['f_58'] = in_text ( 58, 20 ) ;
	$out_arr['f_59'] = in_text ( 59, 19 ) ;
	$out_arr['f_84'] = in_text ( 84, 8 ) ;
	$out_arr['f_85'] = in_text ( 85, 5 ) ;
...
function merge_template () {		// merge argument array with template -- e.g., <td> $my_inputs_arr['fn'] </td>
	...
	$out_str = "\n
	<table id = 'outer' style = '{table_outer}  width: 8in;'>
			<tr><td>

	<table id = 'inner_1' ' style = '{$table} width: 8in;'>
	<tr style = 'height: 0px;'>
		<td style = 'width: 25%; background-color: transparent;'> </td>
		<td style = 'width: 55%; background-color: transparent;'> </td>
		<td style = 'width: 20%; background-color: transparent;'> </td>
		</tr>
	<tr>
		<td style = '{$td_heading}'><b>1. Incident Name:</b><br>
			{$my_inputs_arr['f_1']}</td>
		<td  style = '{$td_heading}'><b>2. Operational Period</b><br>
			From {$my_inputs_arr['f_2']} {$my_inputs_arr['f_3']} To {$my_inputs_arr['f_4']} {$my_inputs_arr['f_5']} </td>
		<td  style = '{$td_heading}'> <b>INDIVIDUAL LOG<br>ICS 214a-OS</b></td>
		</tr>
	</table>\n
		</td></tr><tr><td>
	<table id = 'inner_2' style = '{$table} width: 8in;'>
	<tr style = 'height: 0px;'>
	...
	<tr>
		<td style = '{$td_heading}'><b>3. Individual Name:</b><br>{$my_inputs_arr['f_6']}</td>
		<td style = '{$td_heading}'><b>4. ICS Section</b><br>{$my_inputs_arr['f_7']}</td>
		<td style = '{$td_heading}'><b>5. Assignment/Location</b><br>{$my_inputs_arr['f_8']}</td>
		</tr>
	</table>\n
		</td></tr><tr><td>
	<table id = 'inner_3' style = '{$table} width: 8in;'>
		<tr style = 'height:0px;'>
		<td style = 'width: 20%; background-color: transparent;'></td>
		<td style = 'width: 80%; background-color: transparent;'></td>
	</tr>
	<tr style = '{$tr_thin}'>
		<td colspan=5 style = '{$td_heading}'>&nbsp;6. Activity Log:</td>
		</tr>\n

	<tr style = '{$tr_thin}'>
		<td colspan=1 style = '{$td_plain}'>&nbsp;Time</td>
		<td colspan=1 style = '{$td_plain}'>&nbsp;Major Events</td>
		</tr>\n";
	...
	for ( $i=0; $i< ( 2*24 ) ; $i+=2 ) {					// 2 cols, 24 rows ( 0-23 )
		...
		$out_str .= "</table>\n
			...
			<td colspan=1><b>Name:</b><br>{$my_inputs_arr['f_57']}</td>
			<td colspan=1><b>Position/Title:</b><br>{$my_inputs_arr['f_58']}</td>
			<td colspan=1><b>Signature:</b><br>{$my_inputs_arr['f_59']}</td>
			</tr>
		<tr style = '{$tr_thin}'>
			<td colspan=4 style = '{$td_heading} text-align: center'>Individual Log - ICS 214A, Page 1</td>
			</tr>\n
		</table><br>\n
		</td></tr></table><!-- outer --> \n";
...
<script>
	function init () {
		...
		document.main_form.f_57.value = '<?php echo $the_by;?>';
		do_focus ();
		}		// end function init ()
	</script>
