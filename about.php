...
Changes for 3.31 - New Features and fixes

339.  Added Warehouse functionality to Facilities. Facility can be set as Warehouse and if a Facility user is set up for
      this Facility, then stock control functionality replaces normal facility portal functionality.
340.  Added Public request page - public requests come into the "requests" screen like other service user requests but are
      flagged differently and have the ability to state things such as "immediate danger to life". Public requesters receive
	  email confirmations and updates.
341.  New incident screen now users local user timezone to populate run start time rather than server time.
342.  On the config page under regions there is now an "All Hands" button. This is a temporary override for regions so that 
      all users see all incidents and all resources. Click to set all hands "on", user gets a popup and a flag in the top bar.
	  the session details are changed for "user groups" so that the screens are automatically updated for any logged in users.
343.  On the Personnel Page, Superadmins and Admins see all personnel, Team Managers only see the members that are in teams
      they manage.
