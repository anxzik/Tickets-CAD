...
$showfields = explode(',', get_mdb_variable('contact_list_fields'));
$showfieldnames = array();
foreach($showfields as $val) {
	$showfieldnames[] = get_field_name('member', $val);
	}
$fieldtypes = array();
$col_headings = array();
...
while ($row = mysql_fetch_assoc($result)) {
	foreach ($row as $col_name => $cell) {
		...
		if($col_name != "_by" && $col_name != "_on" && $col_name != "_from" && $col_name != "id") {
			...
			if($col_name != "Not Used" && in_array($fieldindex, $showfields)) {
				$col_headings[$fieldindex] = $col_name . "\t";
				}
			}
		}
	}
...
if($report != 8) {
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		$temp = array();
		foreach ($row as $col_name => $cell) {
			if(in_array($col_name, $showfieldnames)) {
				if($col_name != "_by" && $col_name != "_on" && $col_name != "_from" && $col_name != "id") {
					if($col_name != 'id') {
					...
					if($col_name != "Not Used") {
						if($col_name == "Picture") {
							...
							$temp[] = "<TD class='td_data_wrap text'><img src='" . $cell . "' alt='Member Picture' height='100'></TD>";
							} else {					
							...
							if($col_name == "Member Type") {
								$temp[] = "<TD class='td_data_wrap text'>" . get_memberType($cell) . "</TD>";
								} elseif($col_name == "Member Status") {
								$temp[] = "<TD class='td_data_wrap text'>" . get_memberStatus($cell) . "</TD>";
								} elseif($col_name == "Team") {
								$temp[] = "<TD class='td_data_wrap text'>" . get_Teamname($cell) . "</TD>";
								} elseif($coltype == "DATETIME") {
								$temp[] = "<TD class='td_data_wrap text'>" . format_dateonly($cell) . "</TD>";
								} else {
								$temp[] = "<TD class='td_data_wrap text'>" . $cell . "</TD>";
								}
							}
						}
					}
