...
$manager_team = manager_team($user_id);
$level = $_SESSION['level'];

function output_report($table, $field, $title, $noneflag) {
	global $datetoday, $plusonemonth, $today, $daytoday, $theClass, $manager_team, $level;
	$where2 = ($table == "member" && $manager_team != 0 && $level == $GLOBALS['LEVEL_SUPER'] && $level == $GLOBALS['LEVEL_ADMINISTRATOR']) ? " AND `field3` = " . $manager_team . " " : "";
	$thestring = "";
	$query = "SELECT *, `" . $field . "` FROM `$GLOBALS[mysql_prefix]" . $table . "`
						WHERE (`" . $field . "` BETWEEN '" . $daytoday . "' AND '" . $plusonemonth . "') OR (`" . $field . "` < '" . $daytoday . "') {$where2}";
...
$where = ($manager_team != 0) ? "WHERE `field3` = " . $manager_team . " " : "";
$query = "SELECT `field3`, `field7`, COUNT(*) AS `the_count` FROM `$GLOBALS[mysql_prefix]member` {$where} GROUP BY `field7`";
$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), __FILE__, __LINE__);
$total = 0;
$out_str = "";
while($row = stripslashes_deep(mysql_fetch_assoc($result))) {
...
$query = "SELECT *, `frequency` AS `frequency`, `refresh_due` AS refresh_due,`$GLOBALS[mysql_prefix]allocations`.`id` AS `all_id`,`$GLOBALS[mysql_prefix]member`.`id` AS `member_id` FROM `$GLOBALS[mysql_prefix]allocations`
					...
					WHERE `skill_type` = '1' AND (`frequency` <> 'Permanent') AND ((`refresh_due` BETWEEN '" . $datetoday . "' AND '" . $plusonemonth . "') OR (`refresh_due` < '" . $datetoday . "'))";
$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
if(mysql_num_rows($result) > 0) {
	while ($row = stripslashes_deep(mysql_fetch_assoc($result))) 	{
